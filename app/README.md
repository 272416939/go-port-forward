...
这一点是必须的——服务端只维护一个隧道会话槽位，多个客户端并存会互相抢占，
peer 地址被反复改写，回包发到上一轮的端口，表现为"包在往返但进不去游戏"。

界面是原生窗口，由 WebView2 渲染，显示连接状态、双向字节数与实时速率、每个玩家
IP 的流量，以及运行日志。
...
| 隧道时断时续、包在往返但进不去游戏 | 任务管理器确认只有**一个** `pf-client.exe`。旧版本没有单实例保护，可能堆了多个后台实例互相抢占会话 |
