...
自研迷你加密隧道（UDP + 每用户密钥握手 + X25519 密钥协商 + NaCl 每包加密），
...
在面板侧边栏「用户管理」里创建用户，界面会直接给出该用户的**接入码**——一个
`pf1.` 开头的字符串，内含中转机地址、用户 ID 与隧道密钥。把它发给该用户即可。
...
3. 界面里粘贴**接入码**（凭据会记住，下次启动自动连接）。
   没有接入码时可展开「手动填写」分别填中转机地址、用户 ID 与隧道密钥
...
| 握手一直失败 | 两端放行 UDP 7947；中转机 `tunnel.enabled` 是否开启；服务端日志若提示「协议版本过旧」说明客户端要升级（多用户版握手不兼容旧版），若提示「握手认证失败」说明接入码已失效，在面板重新获取 |
| 界面提示「配置里没有接入凭据」 | 从旧版本升级上来的配置只有地址。在面板「用户管理」复制接入码粘进输入框 |
