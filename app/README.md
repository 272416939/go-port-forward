...
后台跑一晚从 ~23MB 涨到 ~80MB 是**正常且会稳定**的：那是梯级堆（Go 的 GC 节奏）
+ 虚拟网卡读缓冲（~6MB）+ 回程路由缓冲（≤8MB）等**有固定上界**的内存，不是泄漏——
判定标准是斜率：几天内进入平台期即健康，持续线性增长才需要报障。进程设了 96MB 的
软内存上限（GC 节奏阀，不裁任何缓冲）：活内存逼近上限时 GC 会更勤快地把堆压回去。
低配机器可用环境变量调低：`PF_CLIENT_MEMLIMIT_MB=64`（下限 48）。
...
| 界面提示「中转机服务端隧道协议版本过旧（v3）」 | 服务端还没升级（本客户端已升到 v4）。**先升中转机的服务端**，升级完成后本机会自动恢复连接，无需操作 |
| 界面提示「隧道协议版本不匹配」 | 两端版本不一致且对端不是旧版 v3（可能是服务端比客户端新）。确认两端都安装了配套版本的产物 |
