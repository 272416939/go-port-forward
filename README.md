...
| pf-client 一直握手失败 | 中转机 `tunnel.enabled` 是否开启、UDP 7947 是否放行。客户端文案会指明方向：「服务端无应答」= 地址/端口/防火墙问题；「服务端隧道协议版本过旧（v3）」= 中转机还没升级服务端（**先服务端后客户端**，升级完自动恢复）；「隧道协议版本不匹配」= 两端版本不一致，确认都装了配套版本；服务端日志「协议版本过旧」= 客户端要升级，「握手认证失败」= 接入码已失效（面板重新获取） |
