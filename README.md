...
| 两个访问码的「绑定设备」显示同一个设备码 | 两台机器是同一镜像克隆出来的（Windows `MachineGuid` 被原样复制，设备指纹派生自它）。隧道数据面**无影响**（会话密钥与指纹无关，互相解不开）；但「一码绑一设备」的防线对克隆机失效——码可互换使用且服务端无法区分。需要区分：给其中一台重置 `MachineGuid`（注册表 `HKLM\SOFTWARE\Microsoft\Cryptography\MachineGuid`），在面板**解绑**该访问码后重连即自动绑新指纹 |
