...
等不到下一次推送）。**只对这些 IP 添加 /32 路由**进隧道。
Windows 机器的其它上网/RDP 流量、以及"通过后端公网 IP 直接访问"的流量完全不经过隧道。

回收有两条路径，都由「事件建立、时间回收」这一条原则派生：

- 中转机侧 UDP 会话超时（默认 30 秒）后，该来源从活跃列表消失，服务端下发一条
  **会话结束**通知，客户端再等 20 秒删除路由；
- 收不到通知时（客户端断开、控制包丢失）由本地空闲计时兜底，90 秒无入站包即删除。

⚠️ **`/32` 主机路由按目的 IP 生效，与流量是否经隧道无关。** 因此在路由尚未回收的
窗口内，同一玩家 IP 若改用后端公网 IP 直连，回包仍会被吸进隧道而收不到——表现为
「用过代理之后直连进不去，等一会儿又好了」。这是主机路由的固有语义，回收时间已压到
约 1 分钟；需要长期混用两种接入方式的话，让后端只监听虚拟网卡地址、公网 IP 不再
对外提供服务。

### 注意事项 | Notes

- 透明模式与 PROXY 协议透传互斥（二选一）；两者都保留，按拓扑任选。
- **通用模式只转发到公网地址**，目标填隧道网段内任何地址（包括自己的隧道地址、网关）都会被拒绝并引导改用透明模式。TCP 无法经隧道转发（透明模式仅 UDP）；存量此类规则升级后会失效，请迁移或删除。
