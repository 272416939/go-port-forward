...
| 两个访问码的「绑定设备」显示同一个设备码 | 旧版客户端：两台机器是同一镜像克隆的（Windows `MachineGuid` 被原样复制，设备指纹派生自它）。隧道数据面**无影响**（会话密钥与指纹无关）；但「一码绑一设备」的防线对克隆机失效。**新版客户端（2026-09-05 后构建）指纹掺入 SMBIOS 系统UUID**：升级客户端并重连后各机器自动区分（面板设备码会变一次，属预期）；克隆机上后升级的那台会提示「设备不匹配」→ 面板解绑一次即可。无法升级时也可手动重置一台的 `MachineGuid`（注册表 `HKLM\SOFTWARE\Microsoft\Cryptography\MachineGuid`）再解绑重连 |
| pf-client 一直握手失败 | 中转机 `tunnel.enabled` 是否开启、UDP 7947 是否放行。客户端文案会指明方向：「服务端无应答或应答始终无法通过校验」= 地址/端口/防火墙问题，**或同一网络下多台机器同时握手时应答串线**（服务端日志有「同一来源地址已被另一访问码的会话占用」即坐实——重启路由器或错开重连时间）；「服务端隧道协议版本过旧（v3）」= 中转机还没升级服务端（**先服务端后客户端**，升级完自动恢复）；「隧道协议版本不匹配」= 两端版本不一致，确认都装了配套版本；服务端日志「协议版本过旧」= 客户端要升级，「握手认证失败」= 接入码已失效（面板重新获取） |
